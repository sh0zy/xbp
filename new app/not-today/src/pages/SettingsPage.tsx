...
import { WAIT_PRESET_LABELS } from '../types'
import type { WaitPreset } from '../types'
import { Info, Trash2, Database, Package, Clock } from 'lucide-react'

const waitPresets: WaitPreset[] = ['24h', '48h', '72h', '1week', '2weeks']
    ...
    <div className="scrollable" style={{ flex: 1, padding: 16, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <Card>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
          ...
          <p style={{ color: '#64748b', fontSize: 12, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>デフォルト待機時間</p>
        </div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
          {waitPresets.map((preset) => (
            <button
              key={preset}
              onClick={() => updateSettings({ defaultWaitPreset: preset })}
              style={chipStyle(settings.defaultWaitPreset === preset)}
            >
              {WAIT_PRESET_LABELS[preset]}
            </button>
          ))}
        </div>
      </Card>
      ...
      <Card>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <span style={{ color: '#f1f5f9', fontWeight: 600, fontSize: 16 }}>{items.length}</span>
          </div>
          ...
          <button
            ...
            style={{
              ...
              background: '#0f172a', color: '#38bdf8', fontSize: 14,
          ...
          <button
            ...
            style={{
              ...
              color: showClearConfirm ? '#ef4444' : '#64748b', fontSize: 14,
      ...
      <Card>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          <InfoRow label="アプリ名" value="Not Today" />
          <InfoRow label="バージョン" value="1.0.0" />
          <InfoRow label="パッケージ名" value="com.nottoday.app" />
        </div>
      </Card>
    </div>
  )
